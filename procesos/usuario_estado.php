...
if ($_SERVER["REQUEST_METHOD"] !== "POST") {
  go(BASE_URL . "/modulos/usuarios.php?err=" . urlencode("Acceso invalido"));
}
...
verifyCsrfOrRedirect(BASE_URL . "/modulos/usuarios.php?err=" . urlencode("Solicitud invalida. Intenta nuevamente."));

$id = (int)($_POST["id"] ?? 0);
$nuevoEstado = normalizarEstadoUsuario((string)($_POST["estado"] ?? ""));

if ($id <= 0) {
  go(BASE_URL . "/modulos/usuarios.php?err=" . urlencode("Usuario invalido."));
}

if (!estadoUsuarioValido($nuevoEstado)) {
  go(BASE_URL . "/modulos/usuarios.php?err=" . urlencode("Estado invalido."));
}
