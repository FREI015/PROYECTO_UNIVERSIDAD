...
function filtrarPermisosPorTurno(PDO $pdo, array $filas, string $idKey): array {
  if (tieneAlcanceGlobalTurnos()) {
    return $filas;
  }
  ...
  foreach ($filas as $fila) {
    ...
    if (permisoEmpleadoPermitido($pdo, $empleadoId)) {
      $filtradas[] = $fila;
    }
  }
...
if (isset($permisos) && is_array($permisos)) {
  $permisos = filtrarPermisosPorTurno($pdo, $permisos, "empleado_id");
}
